...
class Oci8QueryBuilderTest extends TestCase
{
    ...
    public function test_update_method_with_joins()
    {
        ...
        $builder->getConnection()
            ->shouldReceive('update')
            ->once()
            ->with('update "USERS" set "EMAIL" = ?, "NAME" = ? where ROWID in (select "USERS".ROWID as laravel_rowid from "USERS" inner join "ORDERS" on "USERS"."ID" = "ORDERS"."USER_ID" where "USERS"."ID" = ?)',
                ['foo', 'bar', 1])
            ->andReturn(1);
        $result = $builder->from('users')
    ...
    public function test_joined_write_methods_do_not_mutate_selected_columns_or_bindings()
    {
        $builder = $this->getBuilder();
        $writeSql = 'where ROWID in (select "USERS".ROWID as laravel_rowid from "USERS" inner join "ORDERS" on "USERS"."ID" = "ORDERS"."USER_ID" where "ORDERS"."STATUS" = ?)';

        $builder->getConnection()
            ->shouldReceive('update')
            ->once()
            ->with('update "USERS" set "EMAIL" = ? '.$writeSql, ['updated@example.com', 'pending'])
            ->andReturn(1);
        $builder->getConnection()
            ->shouldReceive('delete')
            ->once()
            ->with('delete from "USERS" '.$writeSql, ['pending'])
            ->andReturn(1);

        $query = $builder->from('users')
            ->selectRaw('? as marker', ['original'])
            ->join('orders', 'users.id', '=', 'orders.user_id')
            ->where('orders.status', '=', 'pending');

        $query->update(['email' => 'updated@example.com']);

        $this->assertSame('select ? as marker from "USERS" inner join "ORDERS" on "USERS"."ID" = "ORDERS"."USER_ID" where "ORDERS"."STATUS" = ?', $query->toSql());
        $this->assertSame(['original', 'pending'], $query->getBindings());
    ...
    public function test_update_method_with_joins_and_limit_on_12c()
    {
        $builder = $this->getBuilder(serverVersion: '12c');
        $builder->getConnection()
            ->shouldReceive('update')
            ->once()
            ->with('update "USERS" "U" set "EMAIL" = ? where ROWID in (select /*+ FIRST_ROWS(2) */ "U".ROWID as laravel_rowid from "USERS" "U" inner join "ORDERS" "O" on "U"."ID" = "O"."USER_ID" where "O"."STATUS" = ? order by "U"."ID" asc offset 1 rows fetch next 2 rows only)',
    ...
    public function test_update_method_with_joins_and_limit_below_12c()
    {
        $builder = $this->getBuilder(serverVersion: '11g');
        $builder->getConnection()
            ->shouldReceive('update')
            ->once()
            ->with('update "USERS" set "EMAIL" = ? where ROWID in (select t2."LARAVEL_ROWID" from ( select rownum AS "rn", t1.* from (select "USERS".ROWID as laravel_rowid from "USERS" inner join "ORDERS" on "USERS"."ID" = "ORDERS"."USER_ID" where "ORDERS"."STATUS" = ? order by "USERS"."ID" asc) t1 ) t2 where t2."rn" between 2 and 3)',
                ['updated@example.com', 'pending'])
            ->andReturn(2);
    ...
    public function test_update_method_with_offset_without_limit_on_12c()
    {
        $builder = $this->getBuilder(serverVersion: '12c');
        $builder->getConnection()
            ->shouldReceive('update')
            ->once()
            ->with(
                'update "USERS" set "EMAIL" = ? where ROWID in (select "USERS".ROWID as laravel_rowid from "USERS" where "STATUS" = ? order by "ID" asc offset 2 rows)',
                ['updated@example.com', 'pending']
            )
            ->andReturn(2);
    ...
    public function test_update_method_with_offset_without_limit_below_12c()
    {
        $builder = $this->getBuilder(serverVersion: '11g');
        $builder->getConnection()
            ->shouldReceive('update')
            ->once()
            ->with(
                'update "USERS" set "EMAIL" = ? where ROWID in (select t2."LARAVEL_ROWID" from ( select rownum AS "rn", t1.* from (select "USERS".ROWID as laravel_rowid from "USERS" where "STATUS" = ? order by "ID" asc) t1 ) t2 where t2."rn" >= 3)',
                ['updated@example.com', 'pending']
            )
            ->andReturn(2);
    ...
    public function test_update_method_with_prefixed_alias_uses_compiled_alias_rowid()
    {
        $builder = $this->getBuilder(prefix: 'app_');
        $builder->getConnection()
            ->shouldReceive('update')
            ->once()
            ->with('update "APP_USERS" "APP_U" set "EMAIL" = ? where ROWID in (select "APP_U".ROWID as laravel_rowid from "APP_USERS" "APP_U" inner join "APP_ORDERS" "APP_O" on "APP_U"."ID" = "APP_O"."USER_ID")',
                ['updated@example.com'])
            ->andReturn(1);
    ...
    public function test_rowid_write_methods_preserve_aliases_containing_whitespace()
    {
        $builder = $this->getBuilder(serverVersion: '12c');
        $builder->getConnection()
            ->shouldReceive('update')
            ->once()
            ->with('update "USERS" "ACTIVE USERS" set "EMAIL" = ? where ROWID in (select /*+ FIRST_ROWS(1) */ "ACTIVE USERS".ROWID as laravel_rowid from "USERS" "ACTIVE USERS" where "STATUS" = ? order by "ID" asc offset 0 rows fetch next 1 rows only)',
                ['updated@example.com', 'pending'])
            ->andReturn(1);

        $result = $builder->from('users as active users')
            ->where('status', '=', 'pending')
            ->orderBy('id')
            ->limit(1)
            ->update(['email' => 'updated@example.com']);
        ...
        $builder->getConnection()
            ->shouldReceive('delete')
            ->once()
            ->with('delete from "USERS" "ACTIVE USERS" where ROWID in (select /*+ FIRST_ROWS(1) */ "ACTIVE USERS".ROWID as laravel_rowid from "USERS" "ACTIVE USERS" where "STATUS" = ? order by "ID" asc offset 0 rows fetch next 1 rows only)', ['inactive'])
            ->andReturn(1);
    ...
    public function test_update_method_with_limit_supports_from_sub_target()
    {
        $builder = $this->getBuilder(serverVersion: '12c');
        $builder->getConnection()
            ->shouldReceive('update')
            ->once()
            ->with(
                'update (select "ID", "EMAIL" from "USERS" where "STATUS" = ?) "ACTIVE_USERS" set "EMAIL" = ? where ROWID in (select /*+ FIRST_ROWS(1) */ "ACTIVE_USERS".ROWID as laravel_rowid from (select "ID", "EMAIL" from "USERS" where "STATUS" = ?) "ACTIVE_USERS" where "ACTIVE_USERS"."ID" > ? order by "ACTIVE_USERS"."ID" asc offset 0 rows fetch next 1 rows only)',
                ['active', 'updated@example.com', 'active', 10]
            )
            ->andReturn(1);

        $result = $builder->fromSub(function ($query) {
            $query->select(['id', 'email'])->from('users')->where('status', '=', 'active');
        }, 'active_users')
            ->where('active_users.id', '>', 10)
            ->orderBy('active_users.id')
            ->limit(1)
            ->update(['email' => 'updated@example.com']);
    ...
    public function test_delete_method_with_limit_on_12c()
    {
        $builder = $this->getBuilder(serverVersion: '12c');
        $builder->getConnection()
            ->shouldReceive('delete')
            ->once()
            ->with('delete from "USERS" where ROWID in (select /*+ FIRST_ROWS(2) */ "USERS".ROWID as laravel_rowid from "USERS" where "STATUS" = ? order by "ID" asc offset 0 rows fetch next 2 rows only)', ['inactive'])
            ->andReturn(2);
    ...
    public function test_delete_method_with_limit_below_12c()
    {
        $builder = $this->getBuilder(serverVersion: '11g');
        $builder->getConnection()
            ->shouldReceive('delete')
            ->once()
            ->with('delete from "USERS" where ROWID in (select t2."LARAVEL_ROWID" from ( select rownum AS "rn", t1.* from (select "USERS".ROWID as laravel_rowid from "USERS" where "STATUS" = ? order by "ID" asc) t1 ) t2 where t2."rn" between 1 and 2)', ['inactive'])
            ->andReturn(2);
    ...
    public function test_delete_method_with_offset_without_limit_on_12c()
    {
        $builder = $this->getBuilder(serverVersion: '12c');
        $builder->getConnection()
            ->shouldReceive('delete')
            ->once()
            ->with(
                'delete from "USERS" where ROWID in (select "USERS".ROWID as laravel_rowid from "USERS" where "STATUS" = ? order by "ID" asc offset 2 rows)',
                ['inactive']
            )
            ->andReturn(2);
    ...
    public function test_delete_method_with_offset_without_limit_below_12c()
    {
        $builder = $this->getBuilder(serverVersion: '11g');
        $builder->getConnection()
            ->shouldReceive('delete')
            ->once()
            ->with(
                'delete from "USERS" where ROWID in (select t2."LARAVEL_ROWID" from ( select rownum AS "rn", t1.* from (select "USERS".ROWID as laravel_rowid from "USERS" where "STATUS" = ? order by "ID" asc) t1 ) t2 where t2."rn" >= 3)',
                ['inactive']
            )
            ->andReturn(2);
    ...
    {
        $builder = $this->getBuilder();
        $builder->getConnection()->shouldReceive('delete')->once()->with('delete from "USERS" where ROWID in (select "USERS".ROWID as laravel_rowid from "USERS" inner join "CONTACTS" on "USERS"."ID" = "CONTACTS"."ID" where "USERS"."EMAIL" = ?)', ['foo'])->andReturn(1);
        $result = $builder->from('users')->join('contacts', 'users.id', '=', 'contacts.id')->where('users.email', '=', 'foo')->delete();
        $this->assertEquals(1, $result);

        $builder = $this->getBuilder(serverVersion: '12c');
        $builder->getConnection()->shouldReceive('delete')->once()->with('delete from "USERS" "A" where ROWID in (select /*+ FIRST_ROWS(1) */ "A".ROWID as laravel_rowid from "USERS" "A" inner join "USERS" "B" on "A"."ID" = "B"."USER_ID" where "EMAIL" = ? order by "ID" asc offset 0 rows fetch next 1 rows only)', ['foo'])->andReturn(1);
        $result = $builder->from('users AS a')->join('users AS b', 'a.id', '=', 'b.user_id')->where('email', '=', 'foo')->orderBy('id')->limit(1)->delete();
        $this->assertEquals(1, $result);

        $builder = $this->getBuilder(serverVersion: '11g');
        $builder->getConnection()->shouldReceive('delete')->once()->with('delete from "USERS" where ROWID in (select t2."LARAVEL_ROWID" from ( select rownum AS "rn", t1.* from (select "USERS".ROWID as laravel_rowid from "USERS" inner join "CONTACTS" on "USERS"."ID" = "CONTACTS"."ID" where "USERS"."ID" = ? order by "ID" asc) t1 ) t2 where t2."rn" between 2 and 2)', [1])->andReturn(1);
        $result = $builder->from('users')->join('contacts', 'users.id', '=', 'contacts.id')->orderBy('id')->offset(1)->take(1)->delete(1);
        $this->assertEquals(1, $result);

        $builder = $this->getBuilder();
        $builder->getConnection()->shouldReceive('delete')->once()->with('delete from "USERS" where ROWID in (select "USERS".ROWID as laravel_rowid from "USERS" inner join "CONTACTS" on "USERS"."ID" = "CONTACTS"."USER_ID" and "USERS"."ID" = ? where "NAME" = ?)', [1, 'baz'])->andReturn(1);
        $result = $builder->from('users')
    ...
    public function test_delete_method_with_prefixed_alias_uses_compiled_alias_rowid()
    {
        $builder = $this->getBuilder(prefix: 'app_');
        $builder->getConnection()
            ->shouldReceive('delete')
            ->once()
            ->with('delete from "APP_USERS" "APP_U" where ROWID in (select "APP_U".ROWID as laravel_rowid from "APP_USERS" "APP_U" inner join "APP_ORDERS" "APP_O" on "APP_U"."ID" = "APP_O"."USER_ID")', [])
            ->andReturn(1);
    ...
    public function test_delete_method_with_join_supports_from_raw_target()
    {
        $builder = $this->getBuilder();
        $builder->getConnection()
            ->shouldReceive('delete')
            ->once()
            ->with(
                'delete from (select "ID", "EMAIL" from "USERS" where "STATUS" = ?) "ACTIVE_USERS" where ROWID in (select "ACTIVE_USERS".ROWID as laravel_rowid from (select "ID", "EMAIL" from "USERS" where "STATUS" = ?) "ACTIVE_USERS" inner join "ORDERS" on "ACTIVE_USERS"."ID" = "ORDERS"."USER_ID" where "ORDERS"."STATUS" = ?)',
                ['active', 'active', 'pending']
            )
            ->andReturn(1);

        $result = $builder
            ->fromRaw('(select "ID", "EMAIL" from "USERS" where "STATUS" = ?) "ACTIVE_USERS"', ['active'])
    ...
    public function test_delete_method_with_limit_duplicates_from_raw_target_bindings()
    {
        $builder = $this->getBuilder(serverVersion: '12c');
        $builder->getConnection()
            ->shouldReceive('delete')
            ->once()
            ->with(
                'delete from (select "ID", "EMAIL" from "USERS" where "STATUS" = ?) "ACTIVE_USERS" where ROWID in (select /*+ FIRST_ROWS(1) */ "ACTIVE_USERS".ROWID as laravel_rowid from (select "ID", "EMAIL" from "USERS" where "STATUS" = ?) "ACTIVE_USERS" where "ACTIVE_USERS"."EMAIL" = ? order by "ACTIVE_USERS"."ID" asc offset 0 rows fetch next 1 rows only)',
        ...
        $result = $builder
            ->fromRaw('(select "ID", "EMAIL" from "USERS" where "STATUS" = ?) "ACTIVE_USERS"', ['active'])
            ->where('active_users.email', '=', 'alice@example.com')
            ->orderBy('active_users.id')
            ->limit(1)
            ->delete();
