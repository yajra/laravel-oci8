...
class SchemaTest extends TestCase
{
    ...
    public function it_can_use_deferrable_constraints_from_schema_builder()
    {
        ...
        Schema::create('compatibility_deferrable_emails', function (Blueprint $table) {
            $table->integer('id')->primary();
            $table->string('email');
            $table->unique('email')
                ->deferrable()
        ...
        DB::table('compatibility_deferrable_emails')->insert([
            ['id' => 1, 'email' => 'alpha@example.test'],
            ['id' => 2, 'email' => 'beta@example.test'],
        ]);

        DB::transaction(function () {
            DB::table('compatibility_deferrable_emails')
                ->where('id', 1)
        ...
        $emails = DB::table('compatibility_deferrable_emails')
            ->orderBy('id')
            ->pluck('email')
            ->all();
