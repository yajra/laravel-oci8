...
class UpdateFromTest extends TestCase
{
    ...
    #[Test]
    public function it_can_update_rows_without_a_join(): void
    {
        if ($this->isMariaDb()) {
            $this->markTestSkipped('MariaDB does not support Laravel updateFrom.');
        }
    ...
    #[Test]
    public function it_can_update_rows_using_only_join_constraints(): void
    {
        if ($this->isMariaDb()) {
            $this->markTestSkipped('MariaDB does not support Laravel updateFrom.');
        }
        ...
        $this->assertDatabaseHas('users', ['id' => 1, 'email' => 'new-1@example.com']);
        $this->assertDatabaseHas('users', ['id' => 2, 'email' => 'new-2@example.com']);
    }

    #[Test]
    public function it_can_update_rows_from_an_unaliased_schema_qualified_target(): void
    {
        if (! (DB::connection() instanceof Oci8Connection)) {
            $this->markTestSkipped('Schema-qualified target reference coverage is Oracle specific.');
        }
        ...
        $updated = DB::table($this->oracleUsername().'.users')
            ->join('profiles', 'profiles.user_id', '=', 'users.id')
            ->where('profiles.active', true)
            ->updateFrom([
                'email' => new Expression('profiles.email'),
            ]);
